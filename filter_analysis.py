...
def optimize_scaling(t, y_al, ext_idx, ref_range=None, k_range=None):
    ...
    ref_range : iterable, optional
        Reference angles to test. If None, 200 values spanning the min and
        max of ``t`` are generated with additional sampling around the mean.
    k_range : iterable, optional
        Scale factors to test. If None, 100 values from 0.5 to 1.5 are used.
    ...
    if ref_range is None:
        ...
        t_max = np.nanmax(t)
        t_mean = np.nanmean(t)
        coarse = np.linspace(t_min, t_max, 100)
        span = (t_max - t_min) / 10.0
        focus_lo = max(t_min, t_mean - span)
        focus_hi = min(t_max, t_mean + span)
        focus = np.linspace(focus_lo, focus_hi, 100)
        ref_range = np.unique(np.concatenate([coarse, focus]))
    if k_range is None:
...
if __name__ == "__main__":
    ...
    # Ensure results directory exists for saving figures
